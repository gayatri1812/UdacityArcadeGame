...
const btnStart = document.querySelector('.btn_new_game');
const btnRestart = document.querySelector('.btn_restart_game');
const btnRestart2 = document.querySelector('.btn_restart_game2');
// DIVs and Layout-Elements
const gameStart = document.querySelector('.start_game');
const canvas = document.querySelector('.canvas_board');
const gameEnd = document.querySelector('.end_game');
//Scoring
let playersMessage = document.querySelector('.players_message');
...
Game.prototype.start = function() {
    ...
    score[0].innerText = 'SCORE: ' + bonusCount;
    score[1].innerText = 'SCORE: ' + bonusCount;
};
...
Player.prototype.doMoves = function() {
    if (this.newxPos != this.xPos || this.newyPos != this.yPos) {
        this.isMoving = true;
        if (this.xPos >= this.newxPos-5 && this.xPos <= this.newxPos + 5)
            this.xPos = this.newxPos;
        if (this.yPos >= this.newyPos-5 && this.yPos <= this.newyPos + 5)
...
function winGame() {
    if (player.yPos <= -15 && bonusCount >= 3 ) {
        game.over();
        playersMessage.innerHTML = `<img src="${player.sprite}" alt="player">`;
        message.innerText = 'Congratulations!! You are saved from the bugs :)';
    }
}
